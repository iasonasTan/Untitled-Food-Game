...
public final class MapHandler extends EntityManager<Tile> {
    ...
    public void move(int stepsX, int stepsY) {
        ...
        if (c_opt.isPresent()) {
            ...
            switch(sd) {
                case RIGHT -> {
                    int d=Math.abs(collider.getWorldX()-context.player.getWidth()-context.player.getWorldX())-5;
                    forEach(e -> e.move(d, 0));
                }
                case LEFT -> {
                    int d=Math.abs(collider.getWorldX()+collider.getWidth()-context.player.getWorldX())+10;
                    forEach(e -> e.move(-d, 0));
                }
            }
        }
